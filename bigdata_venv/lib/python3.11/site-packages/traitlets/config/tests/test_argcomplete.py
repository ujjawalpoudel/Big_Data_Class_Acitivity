...
class TestArgcomplete:
    ...
    def test_complete_custom_completers(self, argcomplete_on):
        ...
        assert set(self.run_completer(app, "app --Application.log_level=")) > {
            "DEBUG",
            "INFO",
        }
        ...
        class CustomCls(Configurable):
            val = Unicode().tag(
                config=True,
                argcompleter=argcomplete.completers.ChoicesCompleter(["foo", "bar"]),
            )
        ...
        assert completions == ["--val=foo", "--val=bar"] or completions == [
            "foo",
            "bar",
        ]
        assert self.run_completer(app, "app --val  --log-level=", point=10) == [
            "foo",
            "bar",
        ]
